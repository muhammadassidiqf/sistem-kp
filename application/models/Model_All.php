...
class Model_All extends CI_Model
{
    ...
    {
        $mhs = $this->model_all->get_mahasiswaid();
        $this->db->select('kp.dosen_pemb as id_dosen, perusahaan.nama as nama_per, kp.penugasan, (select dosen.nama from kp left join dosen on dosen.id_dosen = kp.dosen_pemb where kp.dosen_pemb = id_dosen) as nama_pemb, sidang.id_sidang, sidang.judul, sidang.tanggal, sidang.tgl_pengajuan, (select dosen.nama from sidang left join dosen on dosen.id_dosen = sidang.dosen_png where sidang.dosen_png = dosen.id_dosen) as nama_peng')->from('sidang');
    ...
    public function get_kpid($id)
    {
        ...
        if ($user['role'] == 'Dosen' || $user['role'] == 'Koordinator') {
            ...
            $this->db->join('perusahaan', 'perusahaan.id_perusahaan= (select perusahaan.id_perusahaan from kp join perusahaan on kp.id_perusahaan=perusahaan.id_perusahaan where kp.id_kp = pemeriksa2.id_kp)', 'left');
            $this->db->join('dosen', 'dosen.id_dosen=(select dosen.id_dosen from kp join dosen on kp.id_perusahaan=dosen.id_dosen where kp.dosen_pemb = dosen.id_dosen)', 'left');
            $this->db->where('kp.id_kp=' . $id . ' AND pemeriksa.id_dsn=' . $data['id_dosen'] . ' AND pemeriksa.statuspemeriksa="Menunggu" AND kp.status="Menunggu" AND kp.status2="Menunggu" OR kp.id_kp=' . $id . ' AND pemeriksa.id_dsn=' . $data['id_dosen'] . ' AND pemeriksa.statuspemeriksa="Disetujui" AND kp.status="Disetujui" AND kp.status2="Menunggu" OR kp.id_kp=' . $id . ' AND pemeriksa.id_dsn=' . $data['id_dosen'] . ' AND pemeriksa.statuspemeriksa="Disetujui" AND kp.status="Disetujui" AND kp.status2="Disetujui" OR kp.id_kp=' . $id . ' AND pemeriksa.id_dsn=' . $data['id_dosen'] . ' AND pemeriksa.statuspemeriksa="Tidak Setujui" AND kp.status="Tidak Disetujui" AND kp.status2="Tidak Disetujui" OR kp.id_kp=' . $id . ' AND pemeriksa2.id_dsn=' . $data['id_dosen'] . ' AND pemeriksa2.statuspemeriksa2="Menunggu" AND kp.status="Disetujui" AND kp.status2="Menunggu" OR kp.id_kp=' . $id . ' AND pemeriksa2.id_dsn=' . $data['id_dosen'] . ' AND pemeriksa2.statuspemeriksa2="Disetujui" AND kp.status="Disetujui" AND kp.status2="Menunggu" OR kp.id_kp=' . $id . ' AND pemeriksa2.id_dsn=' . $data['id_dosen'] . ' AND pemeriksa2.statuspemeriksa2="Disetujui" AND kp.status="Disetujui" AND kp.status2="Disetujui" OR kp.id_kp=' . $id . ' AND pemeriksa2.id_dsn=' . $data['id_dosen'] . ' AND pemeriksa2.statuspemeriksa2="Tidak Setujui" AND kp.status="Tidak Disetujui" AND kp.status2="Tidak Disetujui"');
            $kp =  $this->db->get()->result();
        } elseif ($user['role'] == 'Mahasiswa') {
            $data = $this->db->select('*')->from('user')->join('mahasiswa', 'mahasiswa.nrp = user.username', 'left')->where('mahasiswa.nrp = ' . $user['username'] . '')->get()->row_array();
            $this->db->select('kp.id_kp, kp.dosen_pemb as id_dosen, perusahaan.nama as nama_per, kp.penugasan, (select dosen.nama from kp left join dosen on dosen.id_dosen = kp.dosen_pemb where kp.dosen_pemb = id_dosen) as nama_pemb, kp.tanggal, kp.status, kp.status2, (select mahasiswa.nama from mahasiswa where mahasiswa.id_mahasiswa=pengirim.id_mhs) as nama_mhs, (select dosen.nama from dosen where dosen.id_dosen=pemeriksa2.id_dsn) as nama_dsn, pemeriksa.statuspemeriksa, pemeriksa2.statuspemeriksa2')->from('pemeriksa2');
            ...
            $this->db->join('perusahaan', 'perusahaan.id_perusahaan= (select perusahaan.id_perusahaan from kp join perusahaan on kp.id_perusahaan=perusahaan.id_perusahaan where kp.id_kp = pemeriksa2.id_kp)', 'left');
            $this->db->join('dosen', 'dosen.id_dosen=(select dosen.id_dosen from kp join dosen on kp.id_perusahaan=dosen.id_dosen where kp.dosen_pemb = dosen.id_dosen)', 'left');
            $this->db->where('kp.id_kp=' . $id . ' AND pengirim.id_mhs=' . $data['id_mahasiswa'] . ' AND pengirim.statuspengirim="Menunggu" AND kp.status="Menunggu" AND kp.status2="Menunggu" OR kp.id_kp=' . $id . ' AND pengirim.id_mhs=' . $data['id_mahasiswa'] . ' AND pengirim.statuspengirim="Menunggu" AND kp.status="Disetujui" AND kp.status2="Menunggu" OR kp.id_kp=' . $id . ' AND pengirim.id_mhs=' . $data['id_mahasiswa'] . ' AND pengirim.statuspengirim="Disetujui" AND kp.status="Disetujui" AND kp.status2="Disetujui" OR kp.id_kp=' . $id . ' AND pengirim.id_mhs=' . $data['id_mahasiswa'] . ' AND pengirim.statuspengirim="Tidak Setujui" AND kp.status="Tidak Disetujui" AND kp.status2="Tidak Disetujui"');
            $kp =  $this->db->get()->result();
        }
        return $kp;
    }
    ...
    public function get_sidangid($id)
    {
        ...
        if ($user['role'] == 'Dosen' || $user['role'] == 'Koordinator') {
            $data = $this->db->select('*')->from('user')->join('dosen', 'dosen.nik = user.username', 'left')->where('dosen.nik = ' . $user['username'] . '')->get()->row_array();
            $this->db->select('kp.dosen_pemb as id_dosen, perusahaan.nama as nama_per, kp.penugasan, pemeriksa2.id_dsn as pemeriksa2, (select dosen.nama from kp left join dosen on dosen.id_dosen = kp.dosen_pemb where kp.dosen_pemb = dosen.id_dosen) as nama_pemb, (select dosen.nama from sidang left join dosen on dosen.id_dosen = sidang.dosen_png where sidang.dosen_png = dosen.id_dosen) as nama_peng, sidang.id_sidang, sidang.tanggal, sidang.status, sidang.status2, sidang.tgl_pengajuan, sidang.judul, (select mahasiswa.nama from mahasiswa where mahasiswa.id_mahasiswa=pengirim.id_mhs) as nama_mhs, (select dosen.nama from dosen where dosen.id_dosen=pemeriksa2.id_dsn) as nama_dsn, pemeriksa.statuspemeriksa, pemeriksa2.statuspemeriksa2')->from('pemeriksa2');
            $this->db->join('sidang', 'sidang.id_sidang=pemeriksa2.id_sidang', 'left');
            $this->db->join('kp', 'kp.id_kp=sidang.id_kp', 'left');
            $this->db->join('pemeriksa', 'pemeriksa.id_sidang=pemeriksa2.id_sidang', 'left');
            $this->db->join('pengirim', 'pengirim.id_sidang=pemeriksa2.id_sidang', 'left');
            $this->db->join('perusahaan', 'perusahaan.id_perusahaan= (select perusahaan.id_perusahaan from kp join perusahaan on kp.id_perusahaan=perusahaan.id_perusahaan)', 'left');
            $this->db->join('dosen', 'dosen.id_dosen=(select dosen.id_dosen from kp join dosen on kp.id_perusahaan=dosen.id_dosen where kp.dosen_pemb = dosen.id_dosen)', 'left');
            $this->db->where('sidang.id_sidang=' . $id . ' AND pemeriksa.id_dsn=' . $data['id_dosen'] . ' AND pemeriksa.statuspemeriksa="Menunggu" AND sidang.status="Menunggu" AND sidang.status2="Menunggu" OR sidang.id_sidang=' . $id . ' AND pemeriksa.id_dsn=' . $data['id_dosen'] . ' AND pemeriksa.statuspemeriksa="Disetujui" AND sidang.status="Disetujui" AND sidang.status2="Menunggu" OR sidang.id_sidang=' . $id . ' AND pemeriksa.id_dsn=' . $data['id_dosen'] . ' AND pemeriksa.statuspemeriksa="Disetujui" AND sidang.status="Disetujui" AND sidang.status2="Disetujui" OR sidang.id_sidang=' . $id . ' AND pemeriksa.id_dsn=' . $data['id_dosen'] . ' AND pemeriksa.statuspemeriksa="Tidak Setujui" AND sidang.status="Tidak Disetujui" AND sidang.status2="Tidak Disetujui" OR sidang.id_sidang=' . $id . ' AND pemeriksa2.id_dsn=' . $data['id_dosen'] . ' AND pemeriksa2.statuspemeriksa2="Menunggu" AND sidang.status="Disetujui" AND sidang.status2="Menunggu" OR sidang.id_sidang=' . $id . ' AND pemeriksa2.id_dsn=' . $data['id_dosen'] . ' AND pemeriksa2.statuspemeriksa2="Disetujui" AND sidang.status="Disetujui" AND sidang.status2="Menunggu" OR sidang.id_sidang=' . $id . ' AND pemeriksa2.id_dsn=' . $data['id_dosen'] . ' AND pemeriksa2.statuspemeriksa2="Disetujui" AND sidang.status="Disetujui" AND sidang.status2="Disetujui" OR sidang.id_sidang=' . $id . ' AND pemeriksa2.id_dsn=' . $data['id_dosen'] . ' AND pemeriksa2.statuspemeriksa2="Tidak Setujui" AND sidang.status="Tidak Disetujui" AND sidang.status2="Tidak Disetujui"');
            $kp =  $this->db->get()->result();
        } elseif ($user['role'] == 'Mahasiswa') {
            $data = $this->db->select('*')->from('user')->join('mahasiswa', 'mahasiswa.nrp = user.username', 'left')->where('mahasiswa.nrp = ' . $user['username'] . '')->get()->row_array();
            $this->db->select('kp.dosen_pemb as id_dosen, perusahaan.nama as nama_per, kp.penugasan, pemeriksa2.id_dsn as pemeriksa2, (select dosen.nama from kp left join dosen on dosen.id_dosen = kp.dosen_pemb where kp.dosen_pemb = id_dosen) as nama_pemb, (select dosen.nama from sidang left join dosen on dosen.id_dosen = sidang.dosen_png where sidang.dosen_png = dosen.id_dosen) as nama_peng, sidang.id_sidang, sidang.tanggal, sidang.status, sidang.status2, sidang.tgl_pengajuan, sidang.judul, (select mahasiswa.nama from mahasiswa where mahasiswa.id_mahasiswa=pengirim.id_mhs) as nama_mhs, (select dosen.nama from dosen where dosen.id_dosen=pemeriksa2.id_dsn) as nama_dsn, (select dosen.nama from sidang left join dosen on dosen.id_dosen = sidang.dosen_png where sidang.dosen_png = dosen.id_dosen) as nama_peng, pemeriksa.statuspemeriksa, pemeriksa2.statuspemeriksa2')->from('pemeriksa2');
            $this->db->join('sidang', 'sidang.id_sidang=pemeriksa2.id_sidang', 'left');
            $this->db->join('kp', 'kp.id_kp=sidang.id_kp', 'left');
            $this->db->join('pemeriksa', 'pemeriksa.id_sidang=pemeriksa2.id_sidang', 'left');
            $this->db->join('pengirim', 'pengirim.id_sidang=pemeriksa2.id_sidang', 'left');
